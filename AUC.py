...
# threshold = 0.5
# true_labels_binary = (Y_train > threshold).astype(int)

# # Ensure predictions are probabilities
# predictions_prob = tf.keras.activations.sigmoid(predictions)

# # Convert TensorFlow tensors to NumPy arrays and flatten
# true_labels_flat = true_labels_binary.flatten()
# predictions_flat = predictions_prob.numpy().flatten()
...
# Apply threshold to convert raw predictions to binary values
threshold = 0.5
predictions_binary = (tf.keras.activations.sigmoid(predictions) > threshold).numpy().astype(int)

# Assuming true_labels_binary is previously defined
true_labels_binary = (Y_train > threshold).astype(int)

# Squeeze the extra channel from predictions_binary
predictions_binary = predictions_binary.squeeze()

# Calculate pixel-wise accuracy
pixel_accuracy = (predictions_binary == true_labels_binary).mean()
